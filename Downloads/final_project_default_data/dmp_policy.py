...
class DMPPolicyWithPID:
    def __init__(self, square_pose, square_quat, demo_path='demos.npz', dt=0.01, n_bfs=50):
        ...
        for demo_id, demo in demos.items():
            ...
            for i, (start, end) in enumerate(segments):
                ...
                # Apply rotation to both position and quaternion        

                if i == 0:
                    segment_traj -= demo_obj_pos[:, None]
                    #segment_traj[0, :] += 0.013
                    # segment_traj[2, :] -= 0.002
    ...
    def get_action(self, robot_eef_pos: np.ndarray, robot_eef_quat: np.ndarray) -> np.ndarray:
        ...
        # if not rot_done and self.rot_seg0 == False:
        #     delta_pos = [0,0,0] 
        # if rot_done:
        #     self.rot_seg0 = True


        # if self.current_segment == 1 and self.current_step < 15:
        #     self.current_step += 1
        #     print("Waiting for gripper to close at segment 1.")
        #     return np.zeros(7)
        ...
        # if self.current_segment == 0 and np.linalg.norm(delta_pos[:2]) > 0.03:
        #     delta_pos[2] = 0  # only allow XY to move
